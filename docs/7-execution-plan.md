...
| 버전 | 날짜       | 작성자 | 변경 내용                                                                 |
| ---- | ---------- | ------ | ------------------------------------------------------------------------- |
| 1.1  | 2026-05-14 | kimhj  | DB-04·DB-05·BE-09 파일 확장자 .ts→.js 정정, BE-04(Swagger UI) 태스크 추가 |
...
| BE-05 | BE   | 인증 도메인 구현 (UC-01~UC-02b)      | BE-02, BE-03, DB-02                      |
| BE-06 | BE   | 사용자 도메인 구현 (UC-03, UC-12)    | BE-03, BE-05                             |
| BE-07 | BE   | 카테고리 도메인 구현 (UC-04~UC-06)   | BE-03, BE-05, DB-02                      |
| BE-08 | BE   | 할일 도메인 구현 (UC-07~UC-11)       | BE-03, BE-05, BE-07, DB-02               |
| BE-04 | BE   | Swagger UI 구성                      | BE-01                                    |
| BE-09 | BE   | 앱 통합 및 서버 진입점 구성          | BE-03, BE-04, BE-05, BE-06, BE-07, BE-08 |
...
- [x] `psql --version` 실행 시 `psql (PostgreSQL) 17.x` 출력 확인
- [x] `psql -U todoapp_user -d todoapp_db` 접속 성공
- [x] `.env` 파일에 `DATABASE_URL` 항목 작성 완료
...
- [x] `psql ... -f database/schema.sql` 실행 시 오류 없이 완료 (`COMMIT` 출력 확인)
- [x] `\dt` 결과에 `users`, `categories`, `todos`, `refresh_tokens` 4개 테이블 모두 존재
- [x] `\di` 결과에 인덱스 8개 (`idx_todos_user_id`, `idx_todos_category_id`, `idx_todos_user_completed`, `idx_todos_user_due_date`, `idx_categories_user_id`, `idx_refresh_tokens_user_id`, `idx_refresh_tokens_token_hash`, `users_email_key`) 확인
- [x] `\d todos` 에서 `category_id`의 FK 제약이 `ON DELETE RESTRICT`로 표시
- [x] schema.sql 재실행 시 `CREATE TABLE IF NOT EXISTS`로 오류 없이 멱등 실행 확인
...
- [x] `SELECT ... WHERE is_default = true` 결과가 정확히 3행
- [x] 3개 행 모두 `user_id IS NULL` 확인
- [x] `name` 컬럼에 `업무`, `개인`, `쇼핑` 값 각각 존재
- [x] schema.sql 재실행 후 `SELECT COUNT(*) FROM categories WHERE is_default = true` 결과가 여전히 3

---

### DB-04: 백엔드 연결용 pg Pool 설정 (config/db.js 작성)

**태스크명**: `pg` 라이브러리 Pool을 사용한 `config/db.js` 작성 및 연결 테스트

**설명**:
ORM 없이 `pg` 라이브러리의 `Pool`을 사용하여 데이터베이스 연결 모듈을 작성한다.

1. 패키지 설치: `npm install pg`
2. `config/db.js` 작성 내용:
...
3. `server.js` 기동 시 `pool.query('SELECT 1')`로 연결 확인 후 서버 시작

**의존성**: DB-01

**완료 조건**:

- [x] `npm install pg` 완료 및 `package.json` 의존성 등록 확인
- [x] `config/db.js` 파일 생성 및 `Pool` 설정 포함
- [x] `npm run dev` 실행 시 `[DB] 데이터베이스 연결 확인 완료` 로그 출력
- [x] `DATABASE_URL` 미설정 시 명시적 오류 메시지 출력 처리 확인
...
- [x] `scripts/reset-db.js` 파일 생성 및 `NODE_ENV` 프로덕션 가드 포함
- [x] `npm run db:reset` 실행 시 오류 없이 완료되고 성공 메시지 출력
- [x] 리셋 후 `\dt` 로 4개 테이블 재생성 확인
- [x] 리셋 후 기본 카테고리 3개 시드 재삽입 확인
- [x] `NODE_ENV=production npm run db:reset` 실행 시 차단 메시지 출력 후 종료 확인
...
- `src/` 하위 디렉토리 골격 생성: `config/`, `routes/`, `controllers/`, `services/`, `repositories/`, `middlewares/`, `utils/`

**의존성**: 없음

**완료 조건**:

- [x] `npm install` 실행 시 오류 없이 완료되고 `node_modules`가 생성된다
- [x] `.env.example`에 모든 환경변수 키가 명시되어 있다
- [x] `npm run dev` 스크립트가 `nodemon`으로 `src/server.js`를 실행하도록 설정되어 있다
- [x] `src/` 하위 7개 디렉토리가 존재한다

---

### BE-04: Swagger UI 구성

**태스크명**: `swagger-ui-express`를 사용한 API 문서 UI 제공

**설명**:

- 패키지 설치: `npm install swagger-ui-express`
- `swagger/swagger.json` 작성: OpenAPI 3.0.3 전체 API 명세 (Auth, Users, Categories, Todos)
- `src/app.js`에 `/api-docs` 경로로 Swagger UI 마운트
  - `helmet({ contentSecurityPolicy: false })` 적용으로 Swagger UI 스크립트 차단 방지
  - `BearerAuth` security scheme 정의로 JWT 인증 테스트 지원

**의존성**: BE-01

**완료 조건**:

- [x] `npm install swagger-ui-express` 완료 및 `package.json` 의존성 등록 확인
- [x] `swagger/swagger.json` 파일 생성, 전체 엔드포인트 명세 포함
- [x] `GET /api-docs` 접근 시 Swagger UI 정상 렌더링
- [x] Swagger UI에서 JWT 토큰 입력 후 인증이 필요한 엔드포인트 테스트 가능
...
- `src/config/env.js`: `dotenv`로 환경변수 로드, 필수 변수 누락 시 startup에서 즉시 process.exit(1), 검증된 값 export
- `src/config/db.js`: `pg.Pool` 인스턴스 생성 및 export, `DATABASE_URL` 우선 사용 후 개별 변수 fallback, `pool.query` 래퍼 export
- `src/utils/app-error.js`: `AppError extends Error` 클래스 — `statusCode`, `code`, `message` 필드 포함
- `src/utils/jwt-utils.js`: `signAccessToken(payload)`, `signRefreshToken(payload)`, `verifyAccessToken(token)`, `verifyRefreshToken(token)` 함수 구현 — `JWT_ACCESS_SECRET`/`JWT_REFRESH_SECRET` 및 만료 시간 환경변수 사용
- `src/utils/hash-utils.js`: `hashPassword(plain)` (salt rounds ≥ 10), `comparePassword(plain, hashed)` 구현

**의존성**: BE-01

**완료 조건**:

- [x] `env.js`에서 필수 환경변수 누락 시 프로세스 시작 시 명확한 오류 메시지와 함께 종료된다
- [x] `db.js`의 `Pool`이 `DATABASE_URL` 또는 개별 DB 변수로 연결 설정된다
- [x] `AppError` 인스턴스가 `instanceof Error`를 만족하며 `statusCode`, `code`, `message`를 가진다
- [x] `signAccessToken` / `verifyAccessToken` 쌍이 정상 서명·검증되고, 만료 토큰 검증 시 오류를 throw한다
- [x] `hashPassword` 결과가 `comparePassword`로 검증 가능하고, bcrypt salt rounds가 10 이상이다
...
- `src/middlewares/authenticate.js`: `Authorization: Bearer <token>` 헤더 파싱 → `verifyAccessToken` 호출 → 성공 시 `req.user`에 페이로드(`userId`, `email`) 설정, 실패 시 `AppError(401, 'UNAUTHORIZED')` throw
- `src/middlewares/error-handler.js`: Express 4-argument 에러 핸들러, `AppError` 인스턴스면 `{ error: { code, message } }` 형태로 해당 statusCode 응답, 그 외 500 `INTERNAL_SERVER_ERROR` 응답
- `src/middlewares/validate-body.js`: 필수 필드 누락 시 `AppError(400, 'VALIDATION_ERROR')` throw

**의존성**: BE-02

**완료 조건**:

- [x] 유효한 Access Token이 포함된 요청에서 `req.user.userId`와 `req.user.email`이 정확히 설정된다
- [x] 토큰 누락 또는 만료 시 `401` 상태코드와 `{ error: { code: "UNAUTHORIZED", ... } }` 형식으로 응답한다
- [x] `error-handler`가 `AppError`와 일반 `Error`를 구분하여 각각 적절한 statusCode로 응답한다
- [x] 오류 응답 body가 항상 `{ "error": { "code": "...", "message": "..." } }` 형식을 준수한다
- [x] `validate-body` 미들웨어가 필수 필드 누락 시 `400 VALIDATION_ERROR`를 반환한다
...
- `src/repositories/user-repository.js`: `findByEmail`, `create`, `findById`, `updateById`, `deleteById`
- `src/repositories/refresh-token-repository.js`: `save`, `findByToken`, `revokeByToken`, `revokeByUserId`
...
- `src/controllers/auth-controller.js`: 서비스 호출, 응답 형식 통일 (`201` for register, `200` for others)
- `src/routes/auth-routes.js`: `POST /register`, `POST /login`, `POST /logout`, `POST /refresh` 등록

**의존성**: BE-02, BE-03, DB-02

**완료 조건**:

- [x] `POST /api/auth/register`: 신규 이메일로 `201`과 `accessToken`, `refreshToken`, `user` 반환
- [x] 중복 이메일 가입 시 `409` 및 `EMAIL_ALREADY_EXISTS` 코드 반환
- [x] `POST /api/auth/login`: 올바른 자격증명으로 `200`과 토큰 쌍 반환, 잘못된 비밀번호는 `401`
- [x] `POST /api/auth/logout`: refresh 토큰이 DB에서 무효화됨
- [x] `POST /api/auth/refresh`: 유효한 refresh 토큰으로 새 토큰 쌍 발급, 기존 토큰 무효화
- [x] 만료/미존재 refresh 토큰으로 재발급 요청 시 `401` 반환
...
- `src/controllers/user-controller.js`: `getMe(200)`, `updateMe(200)`, `deleteMe(204)`
- `src/routes/user-routes.js`: `GET /me`, `PATCH /me`, `DELETE /me` — 전 라우트 `authenticate` 적용

**의존성**: BE-03, BE-05

**완료 조건**:

- [x] `PATCH /api/users/me`: `name` 변경 시 DB에 반영되고 응답에 `password_hash` 필드 미포함
- [x] `PATCH /api/users/me`: `password` 변경 시 bcrypt 해시 저장, 이후 로그인이 새 비밀번호로 성공
- [x] `DELETE /api/users/me`: 탈퇴 시 `users` 테이블과 `refresh_tokens` 테이블에서 해당 사용자 데이터 모두 삭제
- [x] 인증 토큰 없이 접근 시 `401` 반환
- [x] `req.user.userId` 기준으로만 동작하여 데이터 격리(BR-02) 보장
...
- `src/repositories/category-repository.js`: `findAllByUserId`, `findByIdAndUserId`, `create`, `updateByIdAndUserId`, `deleteByIdAndUserId`, `findFirstDefault`, `reassignTodos(fromId, toId, pgClient)`
...
- `src/controllers/category-controller.js`: `getCategories(200)`, `createCategory(201)`, `updateCategory(200)`, `deleteCategory(204)`
- `src/routes/category-routes.js`: `GET /`, `POST /`, `PATCH /:id`, `DELETE /:id` — 전 라우트 `authenticate` 적용

**의존성**: BE-03, BE-05, DB-02

**완료 조건**:

- [x] `GET /api/categories`: 로그인한 유저의 카테고리만 반환, 타인 카테고리 미포함 (BR-02)
- [x] `POST /api/categories`: 생성 후 `201`과 생성된 객체 반환
- [x] 기본 카테고리(`is_default: true`)에 `PATCH`/`DELETE` 요청 시 `403 CANNOT_MODIFY_DEFAULT` (BR-04/05)
- [x] 카테고리 삭제 시 소속 todos가 첫 번째 기본 카테고리로 이동, 트랜잭션으로 처리 (BR-06)
- [x] 존재하지 않거나 타인 소유 카테고리 접근 시 `404` 반환
...
- `src/repositories/todo-repository.js`: `findAllByUserId(userId, filters)`, `findByIdAndUserId`, `create`, `updateByIdAndUserId`, `toggleComplete`, `deleteByIdAndUserId` — 모든 쿼리에 `user_id` 조건 필수(BR-02)
...
- `src/controllers/todo-controller.js`: `getTodos(200)`, `createTodo(201)`, `updateTodo(200)`, `toggleTodo(200)`, `deleteTodo(204)`
- `src/routes/todo-routes.js`: `GET /`, `POST /`, `PATCH /:id`, `PATCH /:id/toggle`, `DELETE /:id` — 전 라우트 `authenticate` 적용

**의존성**: BE-03, BE-05, BE-07, DB-02

**완료 조건**:

- [x] `GET /api/todos`: `category_id`, `is_completed`, `due_date` 범위 필터 동작, 타인 할일 미반환 (BR-02, BR-10, BR-11)
- [x] `POST /api/todos`: `category_id` 미입력 시 기본 카테고리에 자동 배정
- [x] `PATCH /api/todos/:id/toggle`: `is_completed`가 반전되고 레코드 삭제 없음 (BR-09)
- [x] `PATCH /api/todos/:id`: 지정 필드만 변경, 나머지 유지
- [x] 존재하지 않거나 타인 소유 할일 접근 시 `404` 반환

---

### BE-09: 앱 통합 및 서버 진입점 구성

**태스크명**: app.js, server.js, 라우터 인덱스 구성 및 전체 미들웨어 체인 연결

**설명**:

- `src/routes/index.js`: 각 도메인 라우터를 `/api` prefix로 통합 마운트
- `src/app.js`: Express 앱 생성, `helmet()`, `cors()`, `express.json()` 적용, 라우터 마운트, `error-handler` 마지막 등록, `GET /health` 헬스체크 엔드포인트 추가
- `src/server.js`: `env.js` 로드 → DB 연결 확인 → `app.listen(PORT)`

**의존성**: BE-03, BE-05, BE-06, BE-07, BE-08

**완료 조건**:

- [x] `npm run dev` 실행 시 지정 포트에서 정상 기동되고 로그에 포트 번호 출력
- [x] `GET /health` 요청에 `200` 응답
- [x] 모든 `/api/*` 라우트가 의도한 컨트롤러로 라우팅됨
- [x] 등록되지 않은 경로 요청 시 `404` 반환
- [x] 오류 핸들러가 모든 라우트의 uncaught 오류를 표준 형식으로 응답
...
- `src/__tests__/setup.js`: 테스트 실행 전 `users`, `refresh_tokens` 테이블 TRUNCATE
...
- [x] `npm test` 실행 시 Jest가 JavaScript 파일을 직접 실행
- [x] 각 테스트 실행 전 관련 테이블이 초기화되어 테스트 간 독립성 보장
- [x] 회원가입·로그인·로그아웃·토큰 재발급 정상 시나리오 테스트 모두 통과
- [x] 중복 이메일, 잘못된 비밀번호, 유효하지 않은 토큰 오류 시나리오 테스트 모두 통과
- [x] 토큰 rotation 후 이전 refresh 토큰으로 재발급 요청 시 `401` 반환 검증
...
- [x] 카테고리 테스트 전체 통과, BR-04, BR-05, BR-06 비즈니스 규칙 검증 포함
- [x] 할일 테스트 전체 통과, BR-02(데이터 격리), BR-09(완료 토글) 검증 포함
- [x] 미인증 요청 시 `401` 반환을 모든 보호 엔드포인트에서 확인
- [x] 카테고리 삭제 후 소속 할일의 `category_id`가 기본 카테고리 ID로 변경됨을 DB 조회로 검증
- [x] `npm test` 실행 시 BE-10, BE-11 전체 테스트 스위트 오류 없이 통과
...
- [x] `npm run dev` 실행 시 Vite 개발 서버 정상 구동
- [x] `npm run build` 실행 시 TypeScript 컴파일 오류 없이 빌드 성공
- [x] `npm run test` 실행 시 Vitest 정상 실행
- [x] `@/*` path alias가 Vite와 TypeScript 양쪽에서 동작
- [x] `.env.example` 존재, `.env.local`은 `.gitignore`에 포함
- [x] `src/` 하위 feature-based 디렉토리 구조 생성 완료
...
- [x] `httpClient` 인스턴스로 API 호출 시 `Authorization` 헤더 자동 주입
- [x] 401 응답 시 토큰 갱신 후 원래 요청 자동 재시도
- [x] 토큰 갱신 실패 시 `clearAuth()` 호출 후 `/login` 리다이렉트
- [x] `Button`, `Input`, `ErrorMessage` 컴포넌트가 props에 따라 올바르게 렌더링
- [x] `ApiResponse<T>`, `ApiError` 타입 프로젝트 전체 재사용 가능
- [x] TypeScript strict 모드에서 타입 오류 없음
...
- [x] `useAuthStore`로 `accessToken`, `refreshToken`, `user`, `isAuthenticated` 상태 읽기 가능
- [x] `setAuth()` 호출 후 `isAuthenticated`가 `true`로 변경
- [x] `clearAuth()` 호출 후 모든 상태가 초기값(`null`, `false`)으로 리셋
- [x] `updateTokens()` 호출 시 `user` 상태 유지, 토큰만 교체
- [x] localStorage/sessionStorage에 토큰 데이터 저장 안 됨 확인
- [x] TypeScript strict 모드에서 타입 오류 없음
...
- [x] `useLogin()` 성공 시 `isAuthenticated`가 `true`가 되고 `/todos`로 이동
- [x] `useLogin()` 실패 시 에러 객체 반환, store 상태 미변경
- [x] `useRegister()` 성공 시 인증 상태 설정 완료
- [x] 모든 API 함수가 `ApiResponse<T>` 타입 준수
- [x] `use-token-refresh.ts`가 http-client 인터셉터와 연동 가능한 인터페이스 제공
- [x] TypeScript strict 모드에서 오류 없음
...
- [x] `LoginForm`에서 이메일/비밀번호 입력 후 제출 시 `useLogin` 뮤테이션 호출
- [x] 유효성 검사 실패 시 각 필드 아래에 오류 메시지 표시
- [x] 처리 중 버튼 비활성화, 로딩 상태 표시
- [x] API 에러 발생 시 `ErrorMessage` 컴포넌트로 메시지 표시
- [x] `RegisterForm`에서 비밀번호 불일치 시 제출 차단
- [x] 로그인 ↔ 회원가입 페이지 간 링크 정상 작동
...
- [x] 미인증 상태에서 `/todos` 접근 시 자동으로 `/login` 리다이렉트
- [x] 인증 상태에서 `/login` 접근 시 `/todos` 리다이렉트
- [x] `/` 접근 시 `/todos` 리다이렉트
- [x] `QueryClientProvider`가 앱 전체를 감싸고 하위 컴포넌트에서 훅 사용 가능
- [x] 개발 환경에서 ReactQueryDevtools 패널 표시
- [x] 정의되지 않은 경로 접근 시 적절한 페이지로 리다이렉트
...
- [x] 카테고리 목록이 API에서 fetch되어 `CategoryList`에 렌더링됨
- [x] 카테고리 생성 후 목록 즉시 갱신 (`invalidateQueries`)
- [x] 카테고리 삭제 시 optimistic update로 즉시 제거, 실패 시 복원
- [x] `CategoryItem`에서 삭제 버튼 클릭 시 확인 절차 후 삭제 실행
- [x] `CategoryForm`이 생성/수정 모드를 props로 구분하여 동작
- [x] 로딩/에러 상태 UI 처리
...
- [x] 할일 목록이 `filterParams`에 따라 올바르게 fetch 및 렌더링
- [x] `use-toggle-todo`의 optimistic update로 체크박스 클릭 시 즉각 UI 반응
- [x] `use-delete-todo`의 optimistic update로 삭제 즉시 목록 제거, 실패 시 복원
- [x] `TodoForm`이 생성/수정 모드 구분, 카테고리 선택 드롭다운이 카테고리 목록 표시
- [x] `TodoFilter`에서 필터 변경 시 `filterParams` 갱신 후 목록 재fetch
- [x] 로딩/에러/빈 상태 각각 다른 UI로 표시
...
- [x] 페이지 진입 시 카테고리 목록과 할일 목록 동시 fetch
- [x] 카테고리 선택 시 해당 카테고리의 할일만 필터링
- [x] `TodoFilter`와 카테고리 사이드바 선택이 복합 적용
- [x] 로그아웃 버튼 클릭 시 API 호출 후 store 초기화 및 `/login` 이동
- [x] 할일 추가 폼 열기/닫기 상호작용 정상 동작
- [x] 네비게이션 바의 프로필 링크가 `/profile`로 이동
...
- [x] 페이지 진입 시 현재 사용자 이름이 입력 필드 초기값으로 표시
- [x] 이름 수정 성공 시 auth-store의 `user.name` 업데이트
- [x] 비밀번호 확인 불일치 시 제출 차단 및 오류 메시지 표시
- [x] 회원탈퇴 클릭 시 확인 절차, 확인 후 API 호출 및 로그아웃 실행
- [x] 각 섹션별 API 에러가 해당 섹션에만 표시
- [x] 처리 중 각 섹션의 제출 버튼 비활성화
...
- [x] 320px 뷰포트에서 주요 기능(로그인, 할일 조회/생성)이 가로 스크롤 없이 사용 가능
- [x] 모바일에서 카테고리 목록 접근 UI(탭/메뉴) 존재 및 동작
- [x] 태블릿(768px)에서 사이드바와 메인 영역이 나란히 배치
- [x] 데스크톱(1024px 이상)에서 최대 너비로 중앙 정렬
- [x] 로그인/회원가입 페이지가 태블릿 이상에서 카드 형태로 표시
- [x] 브라우저 개발자 도구 반응형 모드에서 세 가지 브레이크포인트 전환이 자연스러움
...
- [x] `npm run test` 실행 시 모든 테스트 통과
- [x] `auth-store.test.ts`에서 4가지 시나리오 모두 검증
- [x] `http-client.test.ts`에서 401 인터셉트 → 토큰 갱신 → 재시도 흐름 테스트
- [x] `use-login.test.ts`에서 성공/실패 시나리오 모두 검증
- [x] `use-toggle-todo.test.ts`에서 optimistic update 및 롤백 검증
- [x] auth-store와 http-client 인터셉터 로직의 주요 분기 모두 커버

---

_본 실행 계획은 `2-prd.md` v1.3과 `4-project-principles.md` v1.0을 기반으로 작성되었으며, 개발 진행에 따라 업데이트된다._
