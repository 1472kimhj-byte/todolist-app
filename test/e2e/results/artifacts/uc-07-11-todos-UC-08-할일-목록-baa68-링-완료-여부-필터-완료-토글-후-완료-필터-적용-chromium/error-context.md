...
- Name: uc-07-11-todos.spec.ts >> UC-08. 할일 목록 조회 및 필터링 >> 완료 여부 필터: 완료 토글 후 완료 필터 적용
- Location: test\e2e\uc-07-11-todos.spec.ts:63:7
...
Call log:
  - waiting for getByRole('combobox').filter({ hasText: /완료|전체/ }).first()
    - locator resolved to <select aria-label="카테고리 필터">…</select>
  ...
  - generic [ref=e3]:
    ...
    - generic [ref=e13]:
      ...
      - main [ref=e26]:
        ...
        - generic [ref=e36]:
          - button "미완료로 표시" [ref=e37] [cursor=pointer]:
            - img [ref=e38]
          - generic [ref=e41]:
            - generic [ref=e42]: 완료할일_1778806373407
            - generic [ref=e43]:
              - button "완료할일_1778806373407 수정" [ref=e44] [cursor=pointer]: 수정
              - button "완료할일_1778806373407 삭제" [ref=e45] [cursor=pointer]: 삭제
  - generic [ref=e46]:
    - img [ref=e48]
...
```ts
  ...
  3   | 
  4   | async function openTodoCreateDialog(page: import('@playwright/test').Page) {
  5   |   // TodoListPage의 할일 추가 버튼 (aria-label="할일 추가")
  6   |   await page.getByRole('button', { name: '할일 추가' }).click();
  7   |   await expect(page.getByRole('dialog')).toBeVisible({ timeout: 3000 });
  8   | }
  9   | 
  10  | async function createTodo(
  11  |   page: import('@playwright/test').Page,
  12  |   title: string,
  13  |   description?: string,
  14  | ) {
  15  |   await openTodoCreateDialog(page);
  16  |   const dialog = page.getByRole('dialog');
  17  |   await dialog.getByLabel('제목 *').fill(title);
  18  |   if (description) {
  19  |     await dialog.getByPlaceholder('설명 입력 (선택)').fill(description);
  20  |   }
  21  |   // create mode: 버튼 텍스트는 '추가'
  22  |   await dialog.getByRole('button', { name: '추가' }).click();
  23  |   await expect(page.getByText(title)).toBeVisible({ timeout: 5000 });
  24  | }
  25  | 
  26  | test.describe('UC-07. 할일 등록', () => {
